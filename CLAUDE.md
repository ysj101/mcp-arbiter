...
- `ARBITER_MODE=local` — Azure 依存ゼロで起動（Cosmos Emulator / SignalR Emulator / LLM はローカル）
- `ARBITER_MODE=cloud` — Azure 実リソースに接続

開発は **local 優先**。cloud は動作確認・デモ時のみ。

#### 6.2.1 local モードの LLM バックエンド

local モードでは `ARBITER_LLM_BACKEND` で LLM 実装を切り替える。

| 値 | 挙動 | 用途 |
|---|---|---|
| `claude-cli`（既定） | 子プロセスで `claude -p` を起動し、`--output-format json` + `--json-schema` で構造化応答を受け取る。3 審理官の `judge()` は `Promise.all` で並列実行 | 手元で本物の LLM 判定を試す / デモ・プロンプト調整 |
| `mock` | `MockLLMAdapter`（キーワードマッチの簡易ルール） | CLI 未インストール / 未認証環境、Harness、テスト |

claude-cli 経路の前提：

- `claude` CLI がインストールされ、`claude auth` 済みであること
- 子プロセスは `cwd=/tmp` + `--setting-sources user` + `--strict-mcp-config` + 空 MCP config で起動し、プロジェクトの CLAUDE.md / hooks / MCP サーバーは読まれない
- `--tools ""` でツール使用は完全に無効化
- タイムアウトは `CLAUDE_CLI_TIMEOUT_MS`（既定 60 秒）。CLI エラー / タイムアウトは例外化して `/invoke` は 500 を返す

Harness（`pnpm harness:run`）は再現性のため常に `MockLLMAdapter` を使う（[packages/harness/src/runner.ts](packages/harness/src/runner.ts) で固定）。
...
| `ARBITER_LLM_BACKEND` | local LLM 実装 | `claude-cli`（`mock` にフォールバック可） | 参照されない |
| `CLAUDE_CLI_PATH` / `CLAUDE_CLI_MODEL` / `CLAUDE_CLI_TIMEOUT_MS` | claude CLI 設定 | `claude` / `claude-sonnet-4-6` / `60000` | 参照されない |
| `OPENAI_API_KEY` / `AZURE_OPENAI_*` | Azure Foundry 呼出 | 参照されない | 必須 |
...
詳細は [.claude/README.md](.claude/README.md) 参照。チーム共通の permissions / hooks / env で `ARBITER_MODE=local` が既定で入ります。

UI 検証を Claude Code から直接行うための **agent-browser**（[vercel-labs/agent-browser](https://github.com/vercel-labs/agent-browser) — Rust 製ブラウザ自動化 CLI / Skill）と **Skills** の導入・活用パターンは [.claude/AGENT_BROWSER.md](.claude/AGENT_BROWSER.md) にまとめています。Dashboard 変更 PR はこのフローでスクリーンショット付きレビューができます。
